...
import { doc, getDoc, getDocs, collection, onSnapshot, setDoc, updateDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";

const S = { tableId:null, table:null, user:null, isMaster:false, config:null, layout:null, characters:new Map(), members:[], selectedId:null, character:null, editing:false, saving:false };
...
function rerenderMasterSheet(){const m=document.querySelector(".cs-master-fullscreen");if(m)renderCharacter(m);else renderCharacter();}
...
function renderCharacter(rootOverride=null){
  ...
  const c=S.character;const playerCanEdit=!S.isMaster&&(c.editingAllowed!==false&&["draft","rejected","approved"].includes(c.status||"draft"));const masterCanEdit=S.isMaster&&c.status==="approved"&&S.editing;const edit=S.isMaster?masterCanEdit:playerCanEdit;
...
async function load(){
  ...
  S.character=null;
  S.editing=false;
  const t=await getDoc(doc(db,"tables",S.tableId));if(!t.exists())throw Error("Mesa não encontrada.");S.table={id:t.id,...t.data()};if(!A(S.table.members).includes(S.user.uid))throw Error("Você não faz parte desta mesa.");S.isMaster=S.table.ownerId===S.user.uid;S.config=configOf(S.table);S.layout=S.config.layout;
  if(S.isMaster){for(const memberUid of [...new Set(A(S.table.members))]){const u=await getDoc(doc(db,"users",memberUid));if(u.exists())S.members.push({uid:memberUid,...u.data()});}const cs=await getDocs(collection(db,"tables",S.tableId,"characters"));cs.forEach(x=>S.characters.set(x.id,normalize(x.data(),x.id)));S.selectedId=[...S.characters.keys()].find(x=>x!==S.table.ownerId)||null;S.character=S.selectedId?S.characters.get(S.selectedId):null;}
...
async function characterSheetView(params={}){
  ...
  try{await load();const app=document.getElementById("app");app.innerHTML=`<div class="character-sheet-view"><div class="cs-inner"><div class="cs-top"><button class="cs-back">‹</button><div class="cs-title"><h1>⚔️ Ficha de personagem</h1><p>${esc(S.table.name||"Mesa")}</p></div></div><div class="cs-layout"><aside class="cs-picker"></aside><main class="cs-content"></main></div></div></div>`;app.querySelector(".cs-back").onclick=()=>nav("/game/"+encodeURIComponent(S.tableId));renderCharacter();
    ...
    onSnapshot(doc(db,"tables",S.tableId),snap=>{if(snap.exists()){S.table={id:snap.id,...snap.data()};S.config=configOf(S.table);S.layout=S.config.layout;renderCharacter();}});
