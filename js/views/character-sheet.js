...
function currentMaxText(cur,cfg,c,unit=""){
  ...
  if(max===null)return `${current}${unit?` ${unit}`:""}`;
  return `${current}/${max}${unit?` ${unit}`:""}`;
}
...
function renderBoundValue(sec,key,c,edit){
  ...
  cfg.forEach((x,i)=>{
    ...
    if(key==="resources"){
      ...
      const el=input(currentValueOf(cur),(v,node)=>{
      ...
      el.inputMode="decimal"; field.appendChild(el);
    }else{const st=document.createElement("div");st.className="cs-static";st.textContent=currentMaxText(cur,x,c);field.appendChild(st)}
...
function renderEquipment(sec,c,edit){
  ...
  A(c.equipment).forEach((item,i)=>{const card=document.createElement("div");card.className="cs-card cs-equipment-card";
    if(edit){
      ...
      if(set.loadSystem!=="free"){
        if(set.loadSystem==="slot")loadText=item.fractional?`${load.current}/${load.max} ${load.unit}${load.slots>1?` · ${load.slots} slots`:``}`:`1 slot`;
        else loadText=`${load.current}${load.max!==null?`/${load.max}`:""}${load.unit?` ${esc(load.unit)}`:""}`;
      }
...
function renderFinance(sec,c,edit){
  ...
  A(set.currencyTypes).forEach((currency,i)=>{
    ...
    if(edit){const val=input(current,v=>{cur[key]=v});val.inputMode="decimal";f.appendChild(val);}
    else{const st=document.createElement("div");st.className="cs-static";st.textContent=max!==null?`${current}/${max}`:String(current);f.appendChild(st);}
...
function statusText(s){return ({draft:"Rascunho",pending:"Pendente de aprovação",approved:"Aprovada",rejected:"Devolvida para edição",dead:"☠️ Personagem morto"})[s]||"Rascunho";}
...
function masterPicker(root){
  ...
  const npcTitle=document.createElement("div");npcTitle.className="cs-picker-section-title cs-npc-section";npcTitle.textContent=`🎭 NPCs (${npcCharacters().length})`;p.appendChild(npcTitle);
  const npcAlive=npcCharacters().filter(c=>!npcIsDead(c)),npcDead=npcCharacters().filter(npcIsDead);
  npcAlive.forEach(c=>{const b=document.createElement("button");b.type="button";b.className=`cs-player cs-npc-player ${S.selectedNpcId===c.npcId?"selected":""}`;b.innerHTML=`<span><strong>🎭 ${esc(c.profile?.name||"Sem nome")}</strong><small>${S.activeMasterNpcId===c.npcId?"✓ Em uso nas rolagens":"NPC · Vivo"}</small></span>`;b.onclick=()=>openMasterNpcSheet(c.npcId);p.appendChild(b);});
  npcDead.forEach(c=>{const b=document.createElement("button");b.type="button";b.className=`cs-player cs-npc-player ${S.selectedNpcId===c.npcId?"selected":""}`;b.innerHTML=`<span><strong>☠ ${esc(c.profile?.name||"Sem nome")}</strong><small>NPC · Cemitério</small></span>`;b.onclick=()=>openMasterNpcSheet(c.npcId);p.appendChild(b);});
  const npcAdd=document.createElement("button");npcAdd.type="button";npcAdd.className="cs-secondary cs-new-character";npcAdd.textContent="＋ Criar ficha de NPC";npcAdd.onclick=createNpc;p.appendChild(npcAdd);
  const layout=document.createElement("button");layout.type="button";layout.className="cs-primary cs-layout-main";layout.textContent="⚙ Configurar layout";layout.onclick=openLayout;p.appendChild(layout);
}
...
function openMasterNpcSheet(npcId,editing=false){S.selectedNpcId=npcId;S.character=S.npcs.get(npcId)||npcFresh(npcId);S.editing=!!editing;const m=document.createElement("div");m.className="cs-master-fullscreen cs-npc-fullscreen";m.innerHTML=`<div class="cs-master-sheet"><header><button class="cs-close">×</button><div><strong>🎭 ${esc(S.character.profile?.name||"Novo NPC")}</strong><small>NPC · ${npcIsDead(S.character)?"Cemitério":"Ficha do mestre"}</small></div></header><main class="cs-content"></main></div>`;document.body.appendChild(m);m.querySelector(".cs-close").onclick=()=>m.remove();renderCharacter(m,true);}
...
function renderCharacter(rootOverride=null,npcOverride=false){
  ...
  const c=S.character;if(npcOverride){const edit=S.editing||!S.npcs.has(S.selectedNpcId);content.innerHTML=`<div class="cs-character-name-banner cs-character-top-name">🎭 ${esc(c.profile?.name||"Novo NPC")}</div><div class="cs-status"><div><strong>${npcIsDead(c)?"☠ NPC no cemitério":"NPC do mestre"}</strong><small>${S.activeMasterNpcId===S.selectedNpcId?"Esta ficha está sendo usada nas rolagens do mestre.":"Esta ficha está separada das fichas dos players."}</small></div><span>${edit?"Modo edição":"Somente visualização"}</span></div><div class="cs-actions"></div>`;const actions=content.querySelector(".cs-actions");actions.innerHTML=`${S.activeMasterNpcId===S.selectedNpcId?'<button class="cs-secondary" data-npc-use-off>⏹ Parar de usar</button>':(!npcIsDead(c)?'<button class="cs-primary" data-npc-use>🎭 Usar esta ficha</button>':"")}${!edit&&!npcIsDead(c)?'<button class="cs-secondary" data-npc-edit>✏️ Editar ficha</button>':""}${edit?'<button class="cs-primary" data-npc-save>💾 Salvar NPC</button>':""}${!npcIsDead(c)?'<button class="cs-danger" data-npc-dead>☠ Enviar para o cemitério</button>':'<button class="cs-secondary" data-npc-alive>↩ Restaurar NPC</button>'}`;actions.querySelector("[data-npc-use]")?.addEventListener("click",()=>setActiveMasterNpc(S.selectedNpcId));actions.querySelector("[data-npc-use-off]")?.addEventListener("click",()=>setActiveMasterNpc(null));actions.querySelector("[data-npc-edit]")?.addEventListener("click",()=>{S.editing=true;renderCharacter(rootOverride,true)});actions.querySelector("[data-npc-save]")?.addEventListener("click",saveNpc);actions.querySelector("[data-npc-dead]")?.addEventListener("click",()=>toggleNpcDeath(true));actions.querySelector("[data-npc-alive]")?.addEventListener("click",()=>toggleNpcDeath(false));for(const sec of S.layout.sections.filter(x=>x.visible!==false)){const el=renderSection(sec,c,edit);if(el)content.appendChild(el);}return;}
  ...
  if(S.isMaster){
    actions.innerHTML=`${c.status==="pending"?'<button class="cs-primary" data-a="approve">✓ Aprovar</button><button class="cs-danger" data-a="reject">↩ Devolver</button>':""}${c.status==="approved"&&!c.editingAllowed?'<button class="cs-secondary" data-a="unlock">Liberar para o player</button>':""}${c.status==="approved"&&c.editingAllowed?'<button class="cs-secondary" data-a="lock">Bloquear player</button>':""}${c.status==="approved"&&!S.editing&&c.status!=="dead"?'<button class="cs-secondary" data-a="edit">Editar ficha</button>':""}${c.status==="approved"&&S.editing?'<button class="cs-primary" data-save-master>💾 Salvar ficha</button>':""}`;
...
async function characterSheetView(params={}){
  ...
  try{await load();const app=document.getElementById("app");app.innerHTML=`<div class="character-sheet-view"><div class="cs-inner"><div class="cs-top"><button class="cs-back">‹</button><div class="cs-title"><h1>⚔️ Ficha de personagem</h1><p>${esc(S.table.name||"Mesa")}</p></div></div><div class="cs-layout"><aside class="cs-picker"></aside><main class="cs-content"></main></div></div></div>`;app.querySelector(".cs-back").onclick=()=>nav("/game/"+encodeURIComponent(S.tableId));renderCharacter();
