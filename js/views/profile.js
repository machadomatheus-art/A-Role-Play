...
export function render(){
  ...
  $("#settings").onclick=()=>{let m=open("Configurações da conta",`<p style="color:var(--text-secondary);font-size:.82rem">Aqui você pode ajustar os dados básicos da sua conta.</p><label>Nome de usuário</label><input class="mi" id="newname" maxlength="30" value="${esc(me.username)}"><br><br><label>Nova senha</label><input class="mi" id="newpass" type="password" minlength="6" placeholder="Opcional"><p style="color:var(--text-muted);font-size:.72rem">A troca de senha pode exigir login recente.</p>`,`<div class="mf"><button type="button" class="btn" id="logout">Sair</button><button type="button" class="btn primary" id="save">Salvar</button></div>`);m.querySelector("#logout").onclick=async()=>{await signOut(auth);close()};m.querySelector("#save").onclick=async()=>{let n=m.querySelector("#newname").value.trim(),p=m.querySelector("#newpass").value;if(n.length<3||n.length>30)return toast("Nome deve ter entre 3 e 30 caracteres.");try{if(n!==me.username){await updateProfile(u,{displayName:n});await updateDoc(doc(db,"users",u.uid),{username:n});me.username=n}if(p){if(p.length<6)return toast("Senha muito curta.");await updatePassword(u,p)}close();toast("Configurações salvas.")}catch(e){console.error(e);toast(e.code==="auth/requires-recent-login"?"Entre novamente para trocar a senha.":"Não foi possível salvar.")}}};
