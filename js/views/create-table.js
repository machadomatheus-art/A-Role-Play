...
// ============================================================
// Construtor universal de mesas de RPG.
//
...
function buildEquipmentConfig(container) {
  ...
  function equipmentListSetting(title, key, placeholder) {
    ...
    row.className = "builder-inline-add-row";
    row.appendChild(input);
    row.appendChild(add);
    wrap.appendChild(field(title, row, listHelp));
  ...
  function currencyListSetting() {
    ...
    row.appendChild(input);
    row.appendChild(add);
    wrap.appendChild(field("Variedades de moedas", row, { title: "Variedade de moedas", text: `<p>Adicione uma moeda que poderá aparecer na ficha.</p><p><strong>Exemplos:</strong> <code>Ouro</code>, <code>Prata</code>, <code>Créditos</code> ou <code>Fragmentos</code>.</p>` }));
    wrap.appendChild(items);
    renderList();
    return wrap;
  }
      ...
      try {
        ...
        try {
          const avatarSnapshot =
            await getDoc(
              doc(db, "users", uid, "profile", "avatar")
            );

          if (avatarSnapshot.exists()) {
            avatarDataUrl =
              avatarSnapshot.data()?.dataUrl ||
              "";
          }
        } catch (avatarError) {
          console.warn(
            "Não foi possível carregar o avatar do amigo:",
            uid,
            avatarError
          );
        }
        ...
        console.warn(
          "Não foi possível carregar o amigo:",
          uid,
          error
        );
      }
...
function buildFriends(container) {

  const section =
    createSection({
      id: "section-friends",
      title: "Convidar amigos",
      description: "Escolha quem poderá participar desta mesa.",
      icon: "👥",
      help: {
        title: "Participantes da mesa",
        text: `
          <p>
            Todas as mesas são privadas.
          </p>

          <p>
            Somente o mestre pode adicionar ou remover participantes.
          </p>
        `
      }
    });
  ...
  function updateSelectedCounter() {
    ...
    counter.textContent =
      `${state.selectedFriends.length} jogador(es) selecionado(s)`;
  }
  ...
  function renderFriends() {
    ...
    if (
      ...
      empty.textContent =
        state.friends.length === 0
          ? "Você ainda não possui amigos cadastrados."
    ...
    filtered.forEach(
      friend => {
        ...
        checkbox.checked =
          state.selectedFriends.includes(
            friend.uid
          );
        ...
        if (friend.avatarDataUrl) {
          ...
          image.src = friend.avatarDataUrl;
          image.alt = "";
          image.loading = "lazy";
...
async function saveTable(
  ...
  if (
    ...
    showMessage(
      "Você precisa estar autenticado para criar uma mesa.",
      "error"
    );
  ...
  section.innerHTML = `
    ...
    <div class="builder-summary-header">
      ...
      <div>
        ...
        <p>
          Confira a estrutura antes de criar a mesa.
        </p>
    ...
    <div class="builder-summary-grid">
      ...
      <div class="summary-item">
        ...
        <span data-summary="friends">0</span>
      </div>
...
function injectStyles() {
  ...
  compactStyle.textContent = `
    ...
    .dice-builder-grid { display:grid !important; grid-template-columns:repeat(6,minmax(0,1fr)) !important; gap:7px !important; }
    .dice-builder-grid > .dice-builder-option { grid-column:span 2 !important; }
    .dice-builder-grid > :nth-child(7) { grid-column:2 / span 2 !important; }
    .dice-builder-grid > :nth-child(8) { grid-column:4 / span 2 !important; }
  ...
  title.innerHTML = `
    <div class="create-table-header">
      ...
      <div>
        ...
        <p>
          Construa seu próprio RPG, peça por peça.
        </p>
      </div>
    </div>
  `;
  ...
  onAuthStateChanged(
    ...
    user => {
      ...
      if (!user) {
        ...
        showMessage(
          "Você precisa estar logado para criar uma mesa.",
          "error"
        );
