// A Role Play — FCM Web
// Obtém o token FCM e registra o aparelho no Supabase.
...
async function callPushFunction(body) {
  ...
  if (!response.ok || !data?.ok) {
    throw new Error(
      data?.error || `Erro na central de notificações (${response.status}).`
    );
  }
...
async function getARolePlayServiceWorkerRegistration() {
  const swUrl = new URL("../sw.js", import.meta.url).href;
  const appScope = new URL("../", import.meta.url).href;

  console.log("[A Role Play] Registrando Service Worker:", {
    swUrl,
    appScope
  });
...
export async function enableARolePlayPushNotifications() {
  ...
  if (!user) {
    throw new Error("Nenhum usuário do A Role Play está conectado.");
  }

  const registration = await getARolePlayServiceWorkerRegistration();

  console.log(
    "[A Role Play] Permissão atual das notificações:",
...
console.log(
  "[A Role Play] notifications.js carregado. Execute enableARolePlayPushNotifications() para testar."
);

export async function sendARolePlayPush({
  tableId,
  tableName = "A Role Play",
  senderUid = "",
  senderName = "Usuário",
  body = "",
  recipientUids = [],
  ...
  return callPushFunction({
    action: "send_table_message",
    table_id: tableId,
    table_name: tableName,
    sender_uid: senderUid,
    sender_name: senderName,
    body,
    recipient_uids: [...new Set(recipientUids.filter(Boolean))],
    type
  });
}
